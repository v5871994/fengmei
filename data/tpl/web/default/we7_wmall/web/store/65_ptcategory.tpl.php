...
<ul class="nav nav-tabs">
	<li <?php  if($op == 'ptcategory') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('ptcategory');?>">添加驿站商品分类</a></li>
	<li><a href="<?php  echo $this->createWebUrl('ptgoods',array('op'=>'list'));?>">驿站商品列表</a></li>
</ul>
<?php  if($op == 'post') { ?>
<form class="form-horizontal form" id="form1" action="" method="post" enctype="multipart/form-data">
	...
	<div class="main">
		<div class="panel panel-default">
			<div class="panel-heading">添加驿站商品分类</div>
...
<div class="main">
	<div class="panel panel-default">
		<div class="panel-body">
			<a class="btn btn-primary" href="<?php  echo $this->createWebUrl('ptcategory', array('op' => 'post'));?>"/><i class="fa fa-plus-circle"> </i> 添加驿站商品分类</a>
		</div>
	</div>
					...
					<tbody>
						...
						<tr>
							...
							<td style="text-align:right;">
								<a href="<?php  echo $this->createWebUrl('ptgoods', array('op' => 'list', 'cid' => $item['id']))?>" class="btn btn-default btn-sm" title="查看商品" data-toggle="tooltip" data-placement="top"><i class="fa fa-eye"> </i></a>
								<a href="<?php  echo $this->createWebUrl('ptgoods', array('op' => 'post', 'cid' => $item['id']))?>" class="btn btn-default btn-sm" title="添加商品" data-toggle="tooltip" data-placement="top" ><i class="fa fa-plus"> </i></a>
								<a href="<?php  echo $this->createWebUrl('ptcategory', array('op' => 'del', 'id' => $item['id']))?>" class="btn btn-default btn-sm" title="删除" data-toggle="tooltip" data-placement="top" onclick="if(!confirm('删除后该分类下的商品也会删除，确定删除吗?')) return false;"><i class="fa fa-times"> </i></a>
							</td>
						</tr>
						<?php  } } ?>
					</tbody>
...
require(['bootstrap.switch'], function($){
	...
	$(':checkbox[name="status[]"]').on('switchChange.bootstrapSwitch', function(e, state){
		...
		var id = $(this).data('id');
		$.post("<?php  echo $this->createWebUrl('ptcategory', array('op' => 'status'))?>", {status: status, id: id}, function(resp){
			setTimeout(function(){
				location.reload();
			}, 500)
		});
