	...
	<div class="remote-alioss" <?php  if(empty($remote['type']) || $remote['type'] != '2') { ?> style="display:none;" <?php  } ?>>
		...
		<div class="form-group">
			...
			<div class="col-sm-9 col-xs-12">
				...
				<span class="help-block">
					Access Key Secret是您访问阿里云API的密钥，具有该账户完全的权限，请您妥善保管。
				</span>
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-sm-9 col-xs-12">
				...
				<select name="alioss[bucket]" class="form-control">
					...
					<option value="<?php  echo $value['name'];?>@@<?php  echo $value['location'];?>" <?php  if($value['name'] == $remote['alioss']['bucket']) { ?> selected<?php  } ?>><?php  echo $value['name'];?>@@<?php  echo $bucket_datacenter[$value['location']];?></option>
					<?php  } } ?>
				</select>
				<?php  } else { ?>
				<input type="text" name="alioss[bucket]" class="form-control" value="<?php  echo $remote['alioss']['bucket'];?>" placeholder="" />
				<?php  } ?>
				<span class="help-block">
					<?php  if(!empty($bucketContainer)) { ?>
					请注意您在阿里云后台设置的访问权限以及防盗链设置等。
					<?php  } else { ?>
					完善Access Key ID和Access Key Secret资料后可以选择存在的Bucket，否则请手动输入。
					<?php  } ?>
				</span>
			</div>
		</div>
		<div class="form-group">
		 <label class="col-xs-12 col-sm-3 col-md-2 control-label">自定义URL</label>
			<div class="col-sm-9 col-xs-12">
				<input type="text" name="custom[url]" class="form-control" <?php  if(!strexists($remote['alioss']['url'],'aliyuncs.com') && $_W['setting']['remote']['type'] == 2) { ?>value="<?php  echo $remote['alioss']['url'];?>"<?php  } ?> placeholder=""/>
					<span class="help-block">
