...
<div class="main form-horizontal form">
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			<p class="form-control-static" id="check-time"><?php  if(function_exists('gethostbyname')) { ?><i class="fa fa-check text-success"></i> 正常<?php  } else { ?><i class="fa fa-remove text-warning"></i> 异常<?php  } ?></p>
		</div>
	</div>
	<div class="form-group">
		<label class="col-xs-12 col-sm-2 col-md-2 col-lg-1 control-label">云平台域名解析</label>
		<div class="col-sm-10">
			<p class="form-control-static" id="check-time">v2.addons.we7.cc&nbsp;&nbsp;DNS解析的IP：<span id="serverdnsip"><?php  echo gethostbyname('v2.addons.we7.cc')?></span> <?php  if(!empty($_W['setting']['cloudip'])) { ?>手动设置ip：<?php  echo $_W['setting']['cloudip'];?><?php  } ?> <a href="javascript:;"  data-toggle="modal" data-target="#set-server-ip">设置接口IP</a></p>
		</div>
	</div>
	<div class="form-group">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="form-group">
					...
					<div class="col-xs-9">
						<input type="text" class="form-control" name="ip" value="<?php  echo $_W['setting']['cloudip'];?>" />
...
<script>
	...
	$.ajax({
		type : "get",
		data : {'date' : '<?php  echo $_W['timestamp'];?>', 'version' : '<?php  echo IMS_VERSION?>', 'siteurl' : '<?php  echo $_W['siteroot'];?>'},
