		...
		<div class="panel panel-default" id="step1">
			...
			<div class="panel-body">
				<div class="pull-left" style="width:320px;background:#F4F5F9;margin-right:20px;border:1px solid #E7E7EB">
					...
					<div class="shop-detail">
						<ul class="list">
							<li>
								<div class="li-panel">
									<div class="li-content">折扣券设置</div>
				...
				<div class="pull-left alert form" style="width:700px;position:relative;display:block;" id="form1">
					...
					<div class="form-group">
						<label class="col-xs-12 col-sm-3 col-md-2 control-label"><span class="text-danger">*</span> 折扣券标题</label>
						<div class="col-sm-9 col-xs-12">
							<input type="text" class="form-control" name="title"/>
							<div class="help-block">建议填写折扣券“折扣额度”及自定义内容，描述卡券提供的具体优惠。不超过9个字符</div>
						</div>
					</div>
					<div class="form-group">
						<label class="col-xs-12 col-sm-3 col-md-2 control-label"> 折扣券副标题(可选)</label>
						<div class="col-sm-9 col-xs-12">
				...
				<div class="pull-left alert form" style="width:700px;position:relative;" id="form3">
					<div class="arrow_in"></div>
					<h4 style="margin-top:20px;">折扣券详情</h4>
					<div class="form-group">
...
<script>
	...
	$('#coupon-submit').click(function(){
		...
		if(!$.trim($(':input[name="title"]').val())) {
			util.message('折扣券标题不能为空', '','error');
			return false;
		}

		var time_type = $(':radio[name="time_type"]:checked').val();
		if(!time_type) {
			util.message('请完善折扣券有效期', '','error');
			return false;
		}
		...
		$('#coupon-submit').attr('disabled', true);
		$.post("<?php  echo url('wechat/card/discount', array('op' => 'post_save'))?>", {'data' : data}, function(out){
			var out = $.parseJSON(out);
			if(!out.errno) {
				util.message(out.error, '', 'error');
			} else {
				util.message('添加折扣券成功', "<?php  echo url('wechat/card/display');?>", 'success');
			}
			$('#coupon-submit').attr('disabled', false);
			return false;
		});
		return false;
	});
		...
		<div class="panel panel-default" id="step1">
			...
			<div class="panel-body">
				<div class="pull-left" style="width:320px;background:#F4F5F9;margin-right:20px;border:1px solid #E7E7EB">
					<div class="card-title">折扣券</div>
					<div class="card_section" style="position: relative">
						<div class="shop-panel" id="color-purview" style="background:<?php  echo $colors[$item['color']]['value'];?>">
							<div class="logo-area">
								<span class="logo pull-left"><img src="<?php  echo $item['logo_url'];?>" alt=""/></span>
								<div class="pull-left" style="height:38px;line-height:38px"><?php  echo $item['brand_name'];?></div>
								<div class="clear"></div>
							</div>
							<div class="msg-area">
								<div class="tick-msg">
									<p><b id="title-purview"><?php  echo $item['title'];?></b></p>
									<p id="sub-title-purview"><?php  echo $item['sub_title'];?></p>
								</div>
								<p id="sub-time-purview" style="text-align: center">
									<?php  if($item['date_info']['time_type'] == 1) { ?>
										有效期 <?php  echo $item['date_info']['time_limit_start'];?>~<?php  echo $item['date_info']['time_limit_end'];?>
									<?php  } else { ?>
										有效期 领取后<?php  echo $item['date_info']['deadline'];?>天后生效,<?php  echo $item['date_info']['limit'];?>天有效
									<?php  } ?>
								</p>
							</div>
							<div class="deco"></div>
					...
					<div class="card-dispose">
						<div class="destroy_type_preview">
							<?php  if($item['code_type'] == 3) { ?>
								<div class="barcode-area code_preview_3">
									<div class="barcode"></div>
									<p class="code_num">1513-2290-1878</p>
								</div>
							<?php  } else if($item['code_type'] == 2) { ?>
							...
							<?php  } ?>
							<p style="text-align: center" id="notice-purview"><?php  echo $item['notice'];?></p>
						</div>
						<div class="cicon">
							<a href="javascript:;" data-id="form2"><i class="fa fa-pencil"></i></a>
						</div>
					...
					<div class="shop-detail">
						<ul class="list">
							...
							<li>
								<div class="li-panel">
									<div class="li-content">适用门店</div>
									<span class="ricon"><?php  echo $item['location_count'];?>家</span>
									<div class="cicon">
										<a href="javascript:;" data-id="form4"><i class="fa fa-pencil"></i></a>
									</div>
								</div>
					...
					<div class="shop-detail">
						<ul class="list">
							...
							<?php  if(!empty($item['promotion_url_name'])) { ?>
							<li>
								<div class="li-panel" style="border-bottom: none">
									<div class="li-content"><?php  echo $item['promotion_url_name'];?></div>
									<div class="ricon"><span><?php  echo $item['promotion_url_sub_title'];?></span><i class="fa fa-angle-right"></i></div>
									<div class="cicon">
										<a href="javascript:;" data-id="form6"><i class="fa fa-pencil"></i></a>
									</div>
								</div>
				...
				<div class="pull-left alert form" style="width:700px;position:relative;display:block;" id="form1">
					...
					<div class="form-group marbot0">
						...
						<div class="col-sm-9 col-xs-12">
							<p class="form-control-static"><?php  echo $types[$item['type']];?></p>
						</div>
					</div>
					<div class="form-group marbot0">
						<label class="col-xs-12 col-sm-3 col-md-2 control-label"><span class="text-danger"></span> 卡券id</label>
						<div class="col-sm-9 col-xs-12">
							<p class="form-control-static"><?php  echo $item['card_id'];?></p>
						</div>
					</div>
					<div class="form-group marbot0">
						<label class="col-xs-12 col-sm-3 col-md-2 control-label"><span class="text-danger"></span> 适用模块</label>
						<div class="col-sm-9 col-xs-12">
							<p class="form-control-static">
								<?php  if(!empty($item['modules'])) { ?>
									<?php  if(is_array($item['modules'])) { foreach($item['modules'] as $modu) { ?>
										<?php  echo $modu['title'];?> &nbsp;&nbsp;&nbsp;
									<?php  } } ?>
								<?php  } else { ?>
									所有模块适用
					...
					<div class="form-group marbot0">
						...
						<div class="col-sm-9 col-xs-12">
							<p class="form-control-static"><?php  if(!empty($item['sub_title'])) { ?><?php  echo $item['sub_title'];?><?php  } else { ?>无<?php  } ?></p>
						</div>
					</div>
					<div class="form-group marbot0">
						<label class="col-xs-12 col-sm-3 col-md-2 control-label"><span class="text-danger"></span> 卡券额度</label>
						<div class="col-sm-9 col-xs-12">
							<p class="form-control-static"><?php  echo ($item['extra'] / 10);?>折</p>
						</div>
					</div>
					<div class="form-group marbot0">
						...
						<div class="col-sm-9 col-xs-12">
							<p class="form-control-static">
								<?php  if($item['date_info']['time_type'] == 1) { ?>
								<?php  echo $item['date_info']['time_limit_start'];?>~<?php  echo $item['date_info']['time_limit_end'];?>
								<?php  } else { ?>
								领取后<?php  echo $item['date_info']['deadline'];?>天后生效,<?php  echo $item['date_info']['limit'];?>天有效
								<?php  } ?>
							</p>
						</div>
					</div>
					...
					<div class="form-group marbot0">
						...
						<div class="col-sm-9 col-xs-12">
							<p class="form-control-static">
								每个用户限领<?php  echo $item['get_limit'];?>张
							</p>
						</div>
					</div>
					<div class="form-group marbot0">
					...
					<div class="form-group marbot0">
						...
						<div class="col-sm-9 col-xs-12">
							<p class="form-control-static">
								<?php  if($item['can_share']) { ?>用户可以分享领券链接<?php  } else { ?>用户不能分享领券链接<?php  } ?>
							</p>
						</div>
					</div>
					<div class="form-group marbot0">
						...
						<div class="col-sm-9 col-xs-12">
							<p class="form-control-static">
								<?php  if($item['can_give_friend']) { ?>用户领券后可转赠其他好友<?php  } else { ?>用户领券后不能转赠其他好友<?php  } ?>
							</p>
						</div>
					</div>
					<h4>折扣券详情</h4>
					...
					<div class="form-group marbot0">
						...
						<div class="col-sm-9 col-xs-12">
							<p class="form-control-static">
								<?php  echo $item['service_phone'];?>
							</p>
						</div>
					</div>
					<h4>适用门店</h4>
					<?php  if(!empty($item['location_id_list'])) { ?>
						<div class="form-group marbot0">
							...
							<div class="col-sm-9 col-xs-12">
								<table class="table" style="min-width:500px;max-width:500px">
									...
									<tbody>
										<?php  if(is_array($item['location_data'])) { foreach($item['location_data'] as $location) { ?>
											<tr>
												<td><?php  echo $location['business_name'];?></td>
