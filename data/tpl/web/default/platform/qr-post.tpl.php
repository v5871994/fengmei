			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					...
					<div class="form-group">
						...
						<div class="col-sm-9 col-xs-12">
							<label for="radio_1" class="radio-inline"><input type="radio" name="qrc-model" id="radio_1" onclick="$('#model2').hide();$('#model1').show();" value="1" <?php  if(empty($row['model']) || $row['model'] == 1) { ?>checked="checked"<?php  } ?> /> 临时</label>
							<label for="radio_0" class="radio-inline"><input type="radio" name="qrc-model" id="radio_0" onclick="$('#model1').hide();$('#model2').show();" value="2" <?php  if($row['model'] == 2) { ?>checked="checked"<?php  } ?> /> 永久</label>
							<span class="help-block">目前有2种类型的二维码, 分别是临时二维码和永久二维码, 前者有过期时间, 最大为7天（604800秒）, 但能够生成较多数量, 后者无过期时间, 数量较少(目前参数只支持1--100000).</span>
						</div>
					</div>
					<?php  } ?>
					<div class="form-group" id="model1" <?php  if($row['model'] == 2) { ?> style="display:none;"<?php  } ?>>
				...
				<div class="col-sm-9 col-xs-12">
					<input type="text" class="form-control" placeholder="场景值" id="scene_str" name="scene_str" value="<?php  echo $row['scene_str'];?>" />
