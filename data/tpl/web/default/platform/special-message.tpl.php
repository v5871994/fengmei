			...
			<div class="panel-body">
				...
				<div class="form-group">
					...
					<div class="col-xs-12 col-sm-9 col-md-10 col-lg-21">
						<div class="alert alert-info" style="width:auto;">
							...
							<p>文本以外的消息类型, 没有语境很难确定其行为方式. 比如: 没有任何对话, 直接发送过来一张图片或者音频, 我们很难根据图片或者音频来判断对方的目的. 针对这种情况, 【10:10】将这里的扩展操作交给第三方模块来实现. 例如: 对方发送过来一张图片, 并且不再任何对话上下文中, 我们会把这张图片的消息交给能够处理图片消息的默认模块中.</p>
				...
				<div class="form-group">
					...
					<div class="col-xs-12 col-sm-9 col-md-10">
						<select name="<?php  echo $row['type'];?>" class="form-control">
							<option value="" <?php  if(!array_key_exists($row['current'], $mtypes)) { ?>selected<?php  } ?>>不处理(使用系统默认回复)</option>
							<?php  if(is_array($row['handles'])) { foreach($row['handles'] as $item) { ?>
								<option value="<?php  echo $item['name'];?>" <?php  if($row['current'] == $item['name']) { ?>selected<?php  } ?>><?php  echo $item['title'];?></option>
							<?php  } } ?>
						</select>
