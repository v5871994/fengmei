			...
			<?php  if(!empty($recharge)) { ?>
				<div class="form-group clearfix">
					<?php  if(is_array($recharge)) { foreach($recharge as $row) { ?>
						<a href="javascript:;" class="btn btn-warning <?php  if($_GPC['fee'] == $row['recharge']) { ?>btn-danger<?php  } ?> col-lg-3 recharge" data-value="<?php  echo $row['recharge'];?>">充<?php  echo $row['recharge'];?>返<?php  echo $row['back'];?></a>
					<?php  } } ?>
				</div>
			<?php  } ?>
			<div class="form-group">
