...
<div class="ng-cloak panel panel-info ng-container" ng-controller="forgetPanel">
		...
		<div class="tab-content">
			<div class="tab-pane animated active" id="forget">
				...
				<div class="form-group">
					<button class="btn btn-warning btn-block btn-sender" uniacid="<?php  echo $_W['uniacid'];?>">获取验证码</button>
				</div>
			</div>
		</div>
	</form>
	...
	<div class="clearfix actions">
		<a class="pull-left" href="<?php  echo url('auth/register', array('forward' => $_GPC['forward']));?>#wechat_redirect">免费注册</a>
		
		<?php  if($uc_setting['status'] == 1) { ?>
			<a class="pull-right" href="<?php  echo url('auth/login/uc', array('forward' => $_GPC['forward']));?>#wechat_redirect">使用<?php  echo $uc_setting['title'];?>账号登录</a>
			<a class="pull-right">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>
		<?php  } ?>
		<a class="pull-right" href="<?php  echo url('auth/login', array('forward' => $_GPC['forward']));?>#wechat_redirect">立即登录</a>
	</div>
	</div>
</div>
