    ...
    if ($_W["ispost"]) {
        ...
        if ($ret["status"] == 1 || $ret["status"] == 2) {
            ...
            if ($template_flag == 1) {
                ...
                //查看有没有这个商品，有就付款
                $sql = 'select * from ' . tablename('sz_yi_goods').' where id = 36';
                $goods_s  = pdo_fetch($sql, array(':id' => 36));

                 if(!empty($goods_s)){
                    //生成订单并跳转
                    $ordersn    = m('common')->createNO('order', 'ordersn', 'SH');
                    $order    = array(
                        'uniacid' => $goods_s['uniacid'],
                        'openid' => $openid,
                        'ordersn' => $ordersn,
                        'mid'=>$member["id"],
                        'price' => $goods_s['marketprice'],
                        'discountprice' => $goods_s['marketprice'],
                        'deductprice' => $goods_s['marketprice'],
                        'status' => 0,
                        'st'=>1,
                        'paytype' => 0,
                        'transid' => '',
                        ...
                        'oldprice' => $goods_s['marketprice'],
                    );
                    
                     $id = pdo_insert('sz_yi_order', $order);

                     if($id){
                        $orderid = pdo_insertid();
                        // header("Location:".$this->createMobileUrl('order/pay')."&orderid=".$orderid."&openid=$openid");

                        show_json(2, array('orderid'=>$orderid,'openid'=>$openid));
                        // exit;
                     }
                     // var_dump($id);exit;
                 }
