        ...
        if(!empty($_W['openid'])){
            ...
            if(empty($data)){
                if(empty($_W['member']['uid'])){
                    ...
                    if(!empty($fans)){
                        $data = array('uniacid' => $_W['uniacid'], 'uid' => 55 . random(7, true), 'openid' => $fans['openid'], 'nickname' => $fans['nickname'], 'realname' => $fans['nickname'], 'sex' => $fans['sex'], 'avatar' => rtrim($fans['headimgurl'], '0'), 'is_sys' => 2, 'addtime' => TIMESTAMP,);
                        pdo_insert('tiny_wmall_members', $data);
                    }
                }else{
                    $fans = mc_oauth_userinfo();
                    $data = array('uniacid' => $_W['uniacid'], 'uid' => $_W['member']['uid'], 'openid' => $_W['openid'], 'nickname' => $fans['nickname'], 'realname' => $_W['member']['realname'], 'mobile' => $_W['member']['mobile'], 'sex' => $fans['sex'], 'avatar' => rtrim($fans['headimgurl'], '0'), 'is_sys' => 1, 'addtime' => TIMESTAMP,);
                    pdo_insert('tiny_wmall_members', $data);
                }
            }else{
                if(empty($_W['member']['uid'])){
