...
if($op == 'submit') {
	...
	$mission = pdo_fetch("SELECT * FROM ". tablename("goto_history"). " WHERE openid='".$_W['openid']."'");

	if(!empty($mission)&&$_GET['sid']==$mission['shopid']){
		$mission1 = pdo_fetch("SELECT * FROM ". tablename("tiny_wmall_store"). " WHERE id='".$_GET['sid']."'");
		//$jhgjh=pdo_fetch("UPDATE  `a233`.". tablename("goto_history")." SET  `lng` =  ".$mission1['location_y']." lat =  ".$mission1['location_x']." shopid =  ".$_GET['sid']." WHERE  `ims_goto_history`.`id` ='".$mission['id']."'");
		$jhgjh=pdo_update('goto_history', array('shopid' => $_GET['sid'],'lng'=>$mission1['location_y'],'lat'=>$mission1['location_x']), array('id'=>$mission['id'] ));	
	}
	if(empty($mission)){
		$mission1 = pdo_fetch("SELECT * FROM ". tablename("tiny_wmall_store"). " WHERE id='".$_GET['sid']."'");
		
		$sqlsss="INSERT INTO `a233`.". tablename("goto_history")." (`id`, `shopid`, `openid`, `lng`, `lat`) VALUES (NULL, '".$_GET['sid']."', '".$_W['openid']."', '".$mission1['location_y']."', '".$mission1['location_x']."')";
		$madasdf=pdo_fetch($sqlsss);
