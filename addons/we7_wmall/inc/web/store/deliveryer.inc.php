...
if($op == 'pranter')
{
	$devs = pdo_fetchall("SELECT B.id,B.nickname,B.title,B.mobile FROM ". tablename("tiny_wmall_store_deliveryer"). " as A left join ". tablename("tiny_wmall_deliveryer"). " as B ON A.deliveryer_id=B.id WHERE A.uniacid={$_W['uniacid']} and A.sid={$sid} order by B.id");
}
