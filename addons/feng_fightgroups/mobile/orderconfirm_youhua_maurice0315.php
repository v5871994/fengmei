<?php
	...
	if (checksubmit('submit')) {
		$spec_list = pdo_fetchall("select * from " . tablename('ewei_shop_goods_spec') . " where goodsid=:id order by displayorder asc", array(':id' => $goods['goodsid']));
		if($spec_list){
			foreach($spec_list as $v){
				if(empty($item)){
					$item = $_GPC["spec_".$v['id']];
				}else{
					$item .= ','.$_GPC["spec_".$v['id']];
				}
			}
