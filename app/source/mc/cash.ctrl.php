...
if(!empty($type)) {
	...
	if($type == 'credit') {
		...
		if(!empty($log) && $log['status'] == '0') {
			...
			$_tid = pdo_fetchcolumn("SELECT tid FROM ". tablename("core_paylog"). " WHERE uniacid={$_W['uniacid']} AND plid={$log['plid']}");

			//再次更新wmall_order状态
			pdo_update('tiny_wmall_order', array('is_pay'=>1,'status'=>1), array('ordersn' => $_tid));
