<?php
/**
 * [WeEngine System] Copyright (c) 2014 mjjdy321.com
 * WeEngine is NOT a free software, it under the license terms, visited http://www.mjjdy321.com/ for more details.
 */
...
if($_GPC['done'] == '1') {
	$sql = 'SELECT * FROM ' . tablename('core_paylog') . ' WHERE `tid`=:plid';
	$pars = array();
	...
	if(!empty($log)) {
		...
		if(!$log['status']) {
			...
			pdo_update('core_paylog', $record, array('tid' => $log['tid']));

			//再次更新wmall_order状态
			pdo_update('tiny_wmall_order', array('is_pay'=>1), array('ordersn' => $params['tid']));
			//更新财务中心订单状态
			$order = pdo_fetchcolumn("SELECT * FROM ". tablename("tiny_wmall_order"). " WHERE ordersn={$params['tid']}");
			pdo_update('tiny_wmall_order_current_log', array('is_pay' => 1, 'pay_type' => 'wechat'), array('uniacid' => $_W['uniacid'], 'orderid' => $order['id']));

			//手动补充数据，再到原来逻辑筛选去掉重复的
			...
			pdo_insert("tiny_wmall_order_status_log",$log_data);
