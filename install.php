<?php
/**
 * [WeEngine System] Copyright (c) 2014 SDSF1010.CC
 * WeEngine is NOT a free software, it under the license terms, visited http://www.SDSF1010.cc/ for more details.
 */
...
define('APP_URL', 'http://v2.addons.SDSF1010.cc/web/');
define('APP_STORE_URL', 'http://v2.addons.SDSF1010.cc/web');
define('APP_STORE_API', 'http://v2.addons.SDSF1010.cc/api.php');
if($_GET['res']) {
	$res = $_GET['res'];
	$reses = tpl_resources();
...
if($action == 'env') {
	...
	if(version_compare(PHP_VERSION, '5.3.0') == -1) {
		...
		$ret['php']['version']['remark'] = 'PHP版本必须为 5.3.0 以上. <a href="http://bbs.SDSF1010.cc/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58062">详情</a>';
	}
	...
	if($ret['php']['pdo']['ok']) {
		...
		if(!$ret['php']['mysql']['ok']) {
			$ret['php']['pdo']['remark'] = '您的PHP环境虽然不支持 mysql_connect, 但已经支持了PDO, 这样系统是可以正常高效运行的, 不需要额外处理. <a target="_blank" href="http://bbs.SDSF1010.cc/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58073">详情</a>';
		}
	} else {
		if($ret['php']['mysql']['ok']) {
			$ret['php']['pdo']['value'] = '<span class="glyphicon glyphicon-remove text-warning"></span>';
			$ret['php']['pdo']['class'] = 'warning';
			$ret['php']['pdo']['remark'] = '您的PHP环境不支持PDO, 但支持 mysql_connect, 这样系统虽然可以运行, 但还是建议你开启PDO以提升程序性能和系统稳定性. <a target="_blank" href="http://bbs.SDSF1010.cc/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58074">详情</a>';
		} else {
			$ret['php']['pdo']['value'] = '<span class="glyphicon glyphicon-remove text-danger"></span>';
			$ret['php']['pdo']['class'] = 'danger';
			$ret['php']['pdo']['remark'] = '您的PHP环境不支持PDO, 也不支持 mysql_connect, 系统无法正常运行. <a target="_blank" href="http://bbs.SDSF1010.cc/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58074">详情</a>';
	...
	if($ret['php']['curl']['ok']) {
		...
		if(!$ret['php']['fopen']['ok']) {
			$ret['php']['curl']['remark'] = '您的PHP环境虽然不支持 allow_url_fopen, 但已经支持了cURL, 这样系统是可以正常高效运行的, 不需要额外处理. <a target="_blank" href="http://bbs.SDSF1010.cc/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58076">详情</a>';
		}
	} else {
		if($ret['php']['fopen']['ok']) {
			$ret['php']['curl']['value'] = '<span class="glyphicon glyphicon-remove text-warning"></span>';
			$ret['php']['curl']['class'] = 'warning';
			$ret['php']['curl']['remark'] = '您的PHP环境不支持cURL, 但支持 allow_url_fopen, 这样系统虽然可以运行, 但还是建议你开启cURL以提升程序性能和系统稳定性. <a target="_blank" href="http://bbs.SDSF1010.cc/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58086">详情</a>';
		} else {
			$ret['php']['curl']['value'] = '<span class="glyphicon glyphicon-remove text-danger"></span>';
			$ret['php']['curl']['class'] = 'danger';
			$ret['php']['curl']['remark'] = '您的PHP环境不支持cURL, 也不支持 allow_url_fopen, 系统无法正常运行. <a target="_blank" href="http://bbs.SDSF1010.cc/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58086">详情</a>';
	...
	if($ret['php']['ssl']['ok']) {
		...
		$ret['php']['ssl']['remark'] = '没有启用OpenSSL, 将无法访问公众平台的接口, 系统无法正常运行. <a target="_blank" href="http://bbs.SDSF1010.cc/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58109">详情</a>';
	}
	...
	if($ret['php']['gd']['ok']) {
		...
		$ret['php']['gd']['remark'] = '没有启用GD, 将无法正常上传和压缩图片, 系统无法正常运行. <a target="_blank" href="http://bbs.SDSF1010.cc/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58110">详情</a>';
	}
	...
	if($ret['php']['dom']['ok']) {
		...
		$ret['php']['dom']['remark'] = '没有启用DOMDocument, 将无法正常安装使用模块, 系统无法正常运行. <a target="_blank" href="http://bbs.SDSF1010.cc/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58111">详情</a>';
	}
	...
	if($ret['php']['session']['ok'] == 0 || strtolower($ret['php']['session']['ok']) == 'off') {
		...
		$ret['php']['session']['remark'] = '系统session.auto_start开启, 将无法正常注册会员, 系统无法正常运行. <a target="_blank" href="http://bbs.SDSF1010.cc/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58111">详情</a>';
	}
	...
	if($ret['write']['root']['ok']) {
		...
		$ret['write']['root']['remark'] = '本地目录无法写入, 将无法使用自动更新功能, 系统无法正常运行.  <a href="http://bbs.SDSF1010.cc/">详情</a>';
	}
	...
	if($ispost) {
		...
		if(empty($error)) {
			...
			if($_POST['type'] == 'remote') {
				...
				if($ins['error']) {
					die('<script type="text/javascript">alert("链接【10:10】更新服务器失败, 错误为: ' . $ins['error'] . '！");history.back();</script>');
				}
				$archive = $ins['files'];
				if(!$archive) {
				...
				$release = $ins['release'];
				$family = $ins['family'];
								$tmpfile = IA_ROOT . '/SDSF1010source.tmp';
 ...
 * 版本号
 *
 * [WeEngine System] Copyright (c) 2013 SDSF1010.CC
 */
...
VER;
			...
			if(file_exists(IA_ROOT . '/index.php') && is_dir(IA_ROOT . '/web') && file_exists($verfile) && file_exists($dbfile)) {
				...
				die('<script type="text/javascript">alert("你正在使用本地安装, 但未下载完整安装包, 请从【10:10】官网下载完整安装包后重试.");history.back();</script>');
			}
...
function remote_install() {
	...
	$pars['type'] = 'install';
	$pars['product'] = '';
	$url = 'http://v2.addons.SDSF1010.cc/gateway.php';
	$ch = curl_init($url);
...
<html lang="zh-cn">
	<head>
		...
		<title>安装系统 - 【10:10】 - 公众平台自助开源引擎</title>
		...
		<div class="container">
			<div class="header" style="margin:15px auto;">
				<ul class="nav nav-pills pull-right" role="tablist">
					<li role="presentation" class="active"><a href="javascript:;">安装【10:10】系统</a></li>
					<li role="presentation"><a href="http://www.SDSF1010.cc">【10:10】官网</a></li>
					<li role="presentation"><a href="http://bbs.SDSF1010.cc">访问论坛</a></li>
				</ul>
				<img src="?res=logo" />
			</div>
			...
			<div class="footer" style="margin:15px auto;">
				<div class="text-center">
					<a href="http://www.SDSF1010.cc">关于【10:10】</a> &nbsp; &nbsp; <a href="http://bbs.SDSF1010.cc">【10:10】帮助</a> &nbsp; &nbsp; <a href="http://www.SDSF1010.cc">购买授权</a>
				</div>
				<div class="text-center">
					Powered by <a href="http://www.SDSF1010.cc"><b>【10:10】</b></a> v0.7 &copy; 2014 <a href="http://www.SDSF1010.cc">www.SDSF1010.cc</a>
				</div>
			</div>
		</div>
	...
	echo <<<EOF
		<div class="panel panel-default">
			...
			<div class="panel-body" style="overflow-y:scroll;max-height:400px;line-height:20px;">
				<h3>版权所有 (c)2015，【10:10】团队保留所有权利。 </h3>
				<p>
					感谢您选择【10:10】 - 微信公众平台自助开源引擎（以下简称SDSF1010，SDSF1010基于 PHP + MySQL的技术开发，全部源码开放。 <br />
					为了使你正确并合法的使用本软件，请你在使用前务必阅读清楚下面的协议条款：
				</p>
				<p>
					<strong>一、本授权协议适用且仅适用于【10:10】系统(SDSF1010, MicroEngine. 以下简称【10:10】)任何版本，【10:10】官方对本授权协议的最终解释权。</strong>
				</p>
				<p>
					<strong>二、协议许可的权利 </strong>
					<ol>
						<li>您可以在完全遵守本最终用户授权协议的基础上，将本软件应用于非商业用途，而不必支付软件版权授权费用。</li>
						<li>您可以在协议规定的约束和限制范围内修改【10:10】源代码或界面风格以适应您的网站要求。</li>
				...
				<p>
					...
					<ol>
						...
						<li>未经官方许可，禁止在【10:10】的整体或任何部分基础上以发展任何派生版本、修改版本或第三方版本用于重新分发。</li>
				...
				<p>
					...
					<ol>
						...
						<li>电子文本形式的授权协议如同双方书面签署的协议一样，具有完全的和等同的法律效力。您一旦开始确认本协议并安装  SDSF1010，即被视为完全理解并接受本协议的各项条款，在享有上述条款授予的权力的同时，受到相关的约束和限制。协议许可范围以外的行为，将直接违反本授权协议并构成侵权，我们有权随时终止授权，责令停止损害，并保留追究相关责任的权力。</li>
...
function tpl_install_env($ret = array()) {
	...
	echo <<<EOF
		...
		<div class="alert alert-info">系统要求【10:10】整个安装目录必须可写, 才能使用【10:10】所有功能。</div>
		<div class="panel panel-default">
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						...
						<div class="help-block">
							...
							在线安装能够直接安装最新版本【10:10】系统, 如果在线安装困难, 请下载离线安装包后使用本地安装. <br/>
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<input class="form-control" type="text" name="db[name]" value="SDSF1010">
					</div>
				</div>
			</div>
		</div>
...
function tpl_install_finish() {
	...
	<div class="alert alert-success">
		恭喜您!已成功安装“【10:10】 - 公众平台自助开源引擎”系统，您现在可以: <a target="_blank" class="btn btn-success" href="./web/index.php">访问网站首页</a>
	</div>
	<div class="form-group">
		<h5><strong>【10:10】应用商城</strong></h5>
	...
	<div class="alert alert-warning">
		...
		<a target="_blank" class="btn btn-success" href="http://v2.addons.SDSF1010.cc" target="_blank">访问应用商城首页</a>
	</div>
EOF;
	tpl_frame();
}
